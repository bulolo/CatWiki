...
## 2026-06-23 🏗️ 前端架构大重构 + 构建提速 (v1.1.7)

这一版是一次**前端工程化整理**：把管理后台（admin）和站点端（client）从里到外重构了一遍，目标是行为逐字不变、给后续迭代铺好地基。**后端业务逻辑零改动**，同时顺手把 Docker 构建提了速、修了开发环境的两处小坑。

用户 / 开发者能直接感受到的：

- **登录 / 登出 / 切换租户立刻生效**：修复了 admin 认证状态非响应式导致切租户后界面不刷新、缓存不命中的问题
- **Docker 镜像构建明显变快**：重复构建不再每次重新下载依赖
- **`make` 起服务不再因缺网络报错**：首次在新机器上拉起免手动建网

### ✨ 改进

- **认证响应式化**：admin 认证状态改走统一的响应式 store，一处变更全局同步——登录 / 登出 / 切租户即时生效
- **国际化**：admin 与 client 残留的用户可见硬编码文案全部接入 next-intl（英文暂以直译占位，待产品校订）
- **安全**：API Key 改用密码学安全随机数生成；剪贴板复制失败不再假报成功
- **工具链对齐**：admin 对齐 client（React 19 类型、ESLint 9 flat config、zod 4 等），企业版（EE）边界全面类型化

### 🧱 代码重构

把一批上千行的"上帝组件"按职责拆成小件，行为等价。admin 侧涉及文档页、租户管理弹窗、对话流式处理，以及文档导入、数据源、用户管理、站点设置等核心页面；机器人配置、会话分析等也一并梳理。client 侧同样拆了搜索栏、首页与租户门户。配套去除了多处重复逻辑、把请求头拼装、点击外部关闭等通用逻辑抽成复用工具 / hook，并统一了目录与命名约定，admin 业务代码做到无 `any` / `@ts-ignore`。

### ⚡ 构建与运维

- **Docker 构建提速**：后端 / 遥测服务的 Dockerfile 挂载 uv 缓存，重复构建复用依赖
- **Makefile 自动建网**：起服务前自动创建 `catwiki-network`，免手动 `docker network create`

---

## 2026-06-03 👍 用户反馈 + 对话性能透视 (v1.1.6)

这一版围绕**"让对话质量看得见"**做了两件事：用户可以给每条 AI 回答点👍/👎（踩还能填原因），管理员能在后台看到差评的分布和具体内容；同时新增了一个可选的**对话性能面板**，把"这次回答慢在哪"摊开给用户看。后端这一版还顺手把聊天服务整个重构了一遍，并修了一个"客户端关掉页面、后端还在白白算"的浪费。

用户能直接感受到的：

- **每条 AI 回答能点👍/👎了**：点踩可以选个原因（答非所问 / 信息过时 / 等等），反馈会持久化，刷新页面也还在
- **管理员能看差评长什么样**：后台「详细分析 → 用户反馈」新增一个 tab，差评原因分布（环形图）+ 差评消息列表（带 markdown 渲染和详情弹窗），复盘"用户到底对哪类回答不满意"
- **对话性能面板（可选）**：站点设置里打开开关后，对话界面能看到首字延迟、各轮检索耗时、总耗时和 Token 用量；默认关闭，机器人渠道强制关闭
- **关掉聊天页面，后端立刻停手**：以前客户端断连后，后端的 AI 推理还会继续跑到结束（白烧算力和 token），现在断连会立刻取消后端图执行

### 👍 新增：用户反馈系统

客户端 AI 对话的每条回答下方多了👍/👎按钮。点👎时弹出原因选择，反馈实时写库；同一访客对同一条消息只保留一条记录（再点一次切换或取消）。

**为什么用 `(thread_id, message_seq)` 而不是直接传消息行 ID**：用户点👍的瞬间，后端的 background task 可能还没把这轮消息落库，前端根本拿不到数据库行 ID。所以前端传"第几条 assistant 回复"这个稳定坐标，后端再解析成真实消息 ID（带重试兼容写库竞态）。一个 turn 在底层可能拆成多条（工具调用 + 工具结果 + 最终回答），`message_seq` 抽象掉中间步骤，精确定位到最终那条回复。

管理后台「详细分析 → 用户反馈」tab（企业版）：
- 顶部 KPI 行 + 差评原因**环形图分布**
- 差评消息列表：卡片预览用 Streamdown 渲染 markdown（保留 3 行截断），点开是完整内容 + 原因标签 + 时间的详情弹窗
- 分页条常驻，单页时翻页按钮自动禁用

### 📊 新增：对话性能透视（可选开关）

站点设置新增 `show_pipeline_trace` 开关（**默认关闭**）。打开后，客户端对话界面会展示这次回答的性能数据：

- **TTFB / 首字延迟 / 总耗时**：拆出"后端准备 → 首个数据包 → AI 开始打字 → 全部结束"的各段时间
- **每次工具调用的耗时**：通过 `run_id` 把工具开始/结束精确配对，**并行检索也不会错位**，每个工具卡片显示自己实际花了多久
- **Token 用量**：input / output / total，多轮 ReAct 的多次 LLM 调用自动聚合

这些数据会**跟着历史一起持久化**（复用 `ChatMessage.additional_kwargs`），历史回看时一样能看到。**开关关闭时后端一概不下发、不落库**——连 LangChain 原生写入的 `usage_metadata` 也会被剥离，保证 Tokens 不会从历史里"漏出来"。机器人渠道（钉钉/飞书/企微/Telegram）**强制关闭**，只有站内 / 网页渠道按站点开关走。

### ⚡ 客户端断连即取消后端计算

以前的流式实现里，如果用户中途关掉页面或断网，后端的 `graph.astream_events` 还会继续跑完整个 ReAct 循环——上游 LLM 的 token 照烧不误。

这一版把图执行包进独立的 `asyncio.Task`，通过队列与消费端通信。客户端断连时 `finally` 块直接 `task.cancel()`，把 `CancelledError` 注入到当前正在 await 的 LLM 请求点，**后端立即停手**。对高并发场景下的算力和 token 成本是实打实的节省。

### 🏗️ 聊天服务重构

借这次功能落地，把 chat 服务又梳了一遍：

- **`ChatContext` dataclass**：`initialize_chat_context` 统一返回 `(llm, initial_state, config, tenant_id)`，`thread_id` / `model_name` 收成 `@property`，消除满地散落的 `config["configurable"]["thread_id"]`
- **消除双路径重复**：completions 和 responses 两条流式路径以前各写各的图组装，现在共用 `_generate_graph_chunks` 产出器（消除 4 处 `async with get_checkpointer()` 重复 + 2 个内部闭包）
- **图节点提取为模块级**：解除原本的嵌套闭包，节点函数独立可测
- **tracker 状态机 / 超时保护 / 历史格式化**等中优先级审计项一并处理
- **usage 始终下发修复**：Responses API 按 spec 要求让 `response.completed` 始终携带 usage（前端再按开关决定是否渲染）

### 🐛 关键修复

- **文档学习状态卡在"排队中"不自动刷新**：admin 文档列表的轮询回调里读的是原始 API 数据结构（`{ list }`），而非 `select` 变换后的 `{ documents }`，导致 `refetchInterval` 的判断永远不触发、进度永远不动。已修正。

---

## 2026-05-25 🧹 前端代码大扫除 + Telegram 机器人 (v1.1.5)

这一版没有上重磅功能，主线是**把前端两个子项目（管理后台 / 站点端）从里到外整理了一遍**——21 个 commit，砍掉死代码、把弹窗 / 动画 / 删除确认这些细节统一好。同时新增了 **Telegram 机器人**作为一条新渠道。

用户能直接感受到的：

- **多了 Telegram 这条接入**：填个 Bot Token 就能开，**国内部署也能用**（长轮询模式，不要求公网回调地址）
- **删除/退出等确认弹窗变好看了**：管理后台 16 处、站点端 2 处之前是浏览器原生的灰色提示框（移动端 / 暗色主题下尤其难看），现在统一成项目自己的 Dialog 样式
- **删除文档解析器 / 删除数据源以前点了就消失，现在会先问一下**——这两处原本漏了确认，容易误删
- **删除弹窗动画统一了**：之前部分场景下弹窗会"从右下角飞入"中心（一个隐蔽的动画参数 bug），现在所有删除确认都是从屏幕中心淡入展开
- **"清空全部对话历史"真的清空了**：之前底层缺一句事务提交，可能出现刷新一下又回来的窘境

后端只动了少量修复和新渠道接入，没有架构变更。

### 🤖 新增：Telegram 机器人

跟飞书 / 钉钉 / 企业微信走同一套配置面板（管理后台 → 站点设置 → 机器人），填 Bot Token 后就能让用户在 Telegram 里直接跟你的知识库对话。

为什么走**长轮询**：很多团队后端没有公网，配 Webhook 回调要打洞折腾。长轮询就是后端主动去 Telegram 拉消息，无需让 Telegram 能反向访问你 —— 内网部署直接可用。

### 🎨 前端整理：站点端（client）

站点端这次主要做了四件事：

**1. 把手写的"轮询 / 重试 / 自造缓存"统一到 React Query**

最典型一个例子：对话历史侧栏。以前每次发完消息要刷历史列表，是靠一个"自增计数 state"作为信号往子组件传，子组件 useEffect 监听这个数字、数字一变就重拉。这是 React 圈内有名的反模式——现在换成 React Query 的 `invalidateQueries`，那条"信号通路"整段删除。

工具结果弹窗也一样。以前手写了"调失败重试 3 次、每次间隔 1.5 秒、组件已卸载就别 setState 了"这套，5 个 useState/useRef 来回管理状态机。换成 React Query 一行配置就够。

**2. 修了一个隐蔽的 SSE 错误丢失 bug**

AI 对话走的是流式接口（SSE），错误处理走的是一套独立路径，没接入"密码失效自动清理 token"那条链路。结果是：如果管理员中途改了站点访问密码，正常页面会被弹回密码门，但**正在聊天的页面错误会被静默吞掉**。这次把流式和非流式的错误类型统一了。

**3. 代码规范全仓拉齐**

强制单/双引号、分号、UI 组件 barrel 导入风格通过 ESLint 锁死。一次 `lint:fix` 全仓拉齐了一千多处差异，以后新代码不再容易飘。

**4. 清理了 7 处死代码**

包括"为将来准备但从未启用"的工厂方法、定义了字段但从来没人读的 Message 字段、注释里早就过时的 import 残骸等。

### 🛠️ 前端整理：管理后台（admin）

管理后台体量是站点端的六倍，更早积累了更多技术债。分 P0 / P1 / P2 / P3 四档 8 轮逐步推进：

**死代码 / 规范统一（P0）**：管理后台 16 处浏览器原生 `confirm()` 替换为统一的 ConfirmDialog；50 个文件用脚本批量合并 UI 组件 import 风格；ESLint 加规则后跑 `lint:fix` 一次性拉齐 1401 处差异；清掉 lib/auth.ts 里 12 个"预留但从未启用"的 export（含一整套 CSRF 基础设施和过期检查），lib/env.ts 里 4 个零引用的环境枚举。

**日志工具合并（P1）**：项目里同时有 `logger.ts` 和 `error-handler.ts` 两套日志工具，行为还不一致——前者用 `console.error` 会触发 Next.js 全屏错误遮罩干扰调试，后者用 `console.warn` 规避。合并成一个：开发模式自动走 warn，生产模式走 error 便于接入 Sentry。

**任务面板轮询重写（P2）**：管理后台右下角的"导入任务进度"面板，以前是一个 useEffect 里 `setInterval` 轮询，状态变化重建 timer，逻辑很缠。重写成基于 React Query 的 `refetchInterval` —— 每个任务跑到完成态自动停止轮询，无需手动管理。同时**修了 TanStack Query v5 升级遗留的 4 处签名错误**（v5 多了一个参数，旧代码靠 `@ts-ignore` 绕过；这次正确补上）。

**`staleTime` 常量化（P2）**：项目里散落着 18 处缓存过期时间字面量，有 `5*60*1000` 也有 `1000*60*5`（同一个值两种写法），还有 `3*60*1000`（孤值）。统一抽成 `STALE_TIME.{SHORT, MEDIUM, LONG, NONE}` 语义常量。

**两个 600+ 行表单页彻底 mutation 化（P3）**：数据源管理页和文档导入对话框，原本各有 7 / 5 处直接 SDK 调用 + 一堆手写的 `isLoading / isSaving / isBrowsing / isUploading / deletingId / deletingKey` loading state（一共 9 个）。全部换成 React Query / mutation hook 后，这 9 个 loading state 全部删除——状态直接从 mutation 自带的 `isPending` 推导。

**消除所有业务侧 `as any`（P3）**：审计出 27 处 `as any`，逐一处理。其中一些是早期注释误判（"这个字段后端不接受"——实际接口 schema 早就更新了）；一些是直接绕过 unknown 类型的偷懒（补了 `getConfigString` 等类型守卫助手）；一些是 next-intl 动态翻译键（换成更窄的类型断言）。仅保留 1 处社区版的占位 stub（设计上需要 any）。

**三个千行文件首轮拆分（P3）**：

| 文件 | 拆分前 | 拆分后 |
|---|---|---|
| `SiteBotSettings.tsx`（站点机器人配置） | 1102 行 | 843 行 |
| `PlatformModal.tsx`（平台管理弹窗，企业版） | 1635 行 | 1421 行 |
| `app/[slug]/documents/page.tsx`（文档管理主页） | 1761 行 | 1636 行 |

总共抽出 3 个新文件 620 行。这些都是"低风险拆分"——把自包含的子组件移到独立文件，主组件的调用接口完全不变。更激进的拆分（按机器人渠道分文件、平台租户管理的三个视图分文件）作为后续 PR 排期。

### 🎯 一个非常隐蔽的动画 bug 修复

修复过程比较值得记录，留个档：

**症状**：管理后台所有删除确认弹窗（删租户、删用户、删数据源等等）在打开时，看起来不是从屏幕中央自然展开，而是**从屏幕右下角飞到中央**。

**根因**：tailwindcss-animate 的 `animate-in` 关键帧动画用一组叫 `--tw-enter-translate-x/y` 的 CSS 变量作为起始位置，**但它不会自动读取元素自身的平移值**。所以原来的 dialog 组件没写 `slide-in-from-left-1/2 slide-in-from-top-1/2` 时，动画起点变成 `translate(0,0)`、终点 `translate(-50%,-50%)` —— 视觉上就是元素从屏幕左上区域飞入屏幕中心，看起来像从右下角斜飞入。修复就是显式锁定动画起点也是 `-50%`，让动画"零位移"，只剩纯粹的淡入和缩放。

**附带修了一个看不到弹窗的 bug**：企业版的"平台管理"模态框是自写的 `position: fixed` 而不是用 Radix Dialog，z-index 是 `100`；项目的 ConfirmDialog 用 Radix 默认 `z-50`。所以在平台模态框里点删除时，确认弹窗会被遮罩盖住、看不见，但点击还是能穿透——典型的"没反应又确实在响应"窘境。把 ConfirmDialog 提到 `z-300` 后解决。

### 🐛 其他关键修复

- **清空全部会话不提交事务**：底层操作漏了 `@transactional()` 装饰器，可能出现"前端显示已清空、刷新后又回来"
- **社区版构建因企业版路由 import 失败炸全部路由**：之前是 generic `Exception` 兜底，社区版缺企业版模块时会牵连其他正常路由失败启动。现在精确区分"模块不存在"（社区版预期行为）和"代码有错"（真问题），仅前者静默跳过
- **AI 对话 setMessages 类型错误**：原本只能直接传新数组，无法 `prev => prev.map(...)` 函数式更新（React 推荐用法），现在补全类型签名

### 🧰 其他

- 提取 `MessageList` 共享组件，消除 `AIChat` 弹窗和 `AIChatLanding` 主页两处重复的消息渲染代码（站点端）
- ChatWidget 顺手清掉未使用 import；`ToolResultDialog` 补传 `siteId` 让分片接口能正确按站点鉴权
- SDK 重新生成同步到 v1.1.5
