...
@router.get(":byDomain/{domain}", response_model=ApiResponse[Site], operation_id="getAdminSiteByDomain")
...
@router.post("", response_model=ApiResponse[Site], status_code=status.HTTP_201_CREATED, operation_id="createAdminSite")
        ...
        if existing_user:
            ...
            if site.id not in current_managed_sites:
                ...
                await crud_user.update(
                    ...
                    obj_in=UserUpdate(
                        managed_site_ids=new_managed_sites,
                        role=new_role
                    )
                )
        else:
