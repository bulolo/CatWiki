...
class Searcher:
    ...
    def search(self, ip: bytes | str):
        ...
        if len(ip_bytes) != self.version.byte_num:
            raise ValueError(
                f"invalid ip address `{util.ip_to_string(ip_bytes)}` ({self.version.name} expected)"
            )
    ...
    def __str__(self):
        return f'{{"version": {self.version.name}, "db_path": "{self.__db_path}", "v_index": {None if self.vector_index is None else len(self.vector_index)}, "c_buffer": {None if self.c_buffer is None else len(self.c_buffer)}}}'
