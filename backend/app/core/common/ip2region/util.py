...
class Header:
    ...
    "indexPolicy": {self.indexPolicy},
    "createdAt": {self.createdAt},
    "startIndexPtr": {self.startIndexPtr},
    "endIndexPtr": {self.endIndexPtr},
    "ipVersion": {self.ipVersion},
    "runtimePtrBytes": {self.runtimePtrBytes}
}}"""
...
class Version:
    def __init__(
        ...
        byte_num: int,
        index_size: int,
        ip_compare: Callable[[bytes, bytes, int], int],
    ):
        self.id = id
        self.name = name
    ...
    def __str__(self):
        return f'{{"id": {self.id}, "name": "{self.name}", "bytes": {self.byte_num}, "index_size": {self.index_size}}}'
