...
class VectorService:
    ...
    async def retrieve(
        ...
        try:
            ...
            logger.debug(f"✅ [Retrieve] 向量召回完成 | 数量: {len(results)} | 耗时: {duration:.3f}s")
            ...
            # 6. 转换为响应对象
            response_objects = [
                VectorRetrieveResponse(**item) for item in final_list
            ]
            ...
            for i, res in enumerate(response_objects):
                ...
                log_lines.append(f"   #{i+1}: {score_str} | Title: {res.document_title[:40] if res.document_title else 'N/A'}")
