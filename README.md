...
| `make setup` | **[开发环境] 一键启动**: 执行 `init-env` 并启动 `dev` 服务 |
| `make init-env` | **[开发环境] 初始化环境**: 清理并重新从 `.env.example` 复制配置文件 |
| `make dev` | **[开发环境] 开发启动**: 构建镜像并在前台启动，实时查看所有服务日志 |
| `make down` | **[开发环境] 优雅停止**: 停止并移除容器，保留数据库存储卷 |
| `make restart` | **[开发环境] 快捷重启**: 仅重启后端应用容器 |
| `make logs` | **[开发环境] 实时日志**: 查看后端核心服务的实时运行日志 |
| `make db-migrate m="msg"` | **[开发环境] 迁移脚本**: 生成新的数据库迁移脚本 (需提供备注 `m`) |
| `make db-psql` | **[开发环境] 数据库终端**: 进入 PostgreSQL 交互式终端 |
| `make gen-sdk` | **[开发环境] 生成 SDK**: 触发后端 API 自动生成前端 SDK |
| `make prod-init` | **[生产环境] 生产初始化**: 初始化生产环境配置文件模板 |
| `make prod-up` | **[生产环境] 生产启动**: 在后台启动生产环境所有服务 |
| `make prod-down` | **[生产环境] 生产停止**: 停止并移除生产环境容器 |
| `make prod-restart` | **[生产环境] 重启后端**: 仅重启生产环境后端应用容器 |
| `make prod-logs` | **[生产环境] 实时日志**: 查看生产环境容器日志 |
| `make prod-clean` | **[生产环境] 深度重置**: 停止容器并**删除生产所有数据卷** (❗危险操作) |
| `make clean` | **[开发环境] 深度重置**: 停止容器并**删除所有数据卷** (清空数据库和存储) |
